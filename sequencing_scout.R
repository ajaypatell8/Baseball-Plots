...
game_pitcher_seq <- pitcher_seq %>% 
  filter(pitcher_name == pitcherName, times_thrown >= 30) %>% 
  arrange(-desc(xwOBA)) %>% 
...
pitcher_seq_table <- game_pitcher_seq %>% 
  ...
  gt_hulk_col_numeric(columns = xwOBA, reverse = TRUE) %>% 
  tab_header(
...
top_three <- batter_seq %>% 
  ...
  mutate(batter_name = factor(batter_name, levels = lineup)) %>% 
  arrange(batter_name)
...
ggsave("/Users/ajaypatel/Downloads/RFiles/baseball/seq_scout.png", final,
